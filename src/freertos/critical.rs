...
impl CriticalRegion {
    pub fn enter() -> Self {
        //unsafe { freertos_rs_enter_critical(mux); }
        // CriticalRegion
        panic!("YXJ");
    }
}
...
impl Drop for CriticalRegion {
    fn drop(&mut self) {
        // unsafe { freertos_rs_exit_critical(mux); }
        panic!("YXJ");
    }
}
...
impl<T> ExclusiveData<T> {
    pub fn new(data: T) -> Self { ExclusiveData { data: UnsafeCell::new(data), } }

    pub fn lock(&self) -> Result<ExclusiveDataGuard<T>, FreeRtosError> {
        Ok(ExclusiveDataGuard { __data: &self.data,
                                __lock: CriticalRegion::enter(), })
    }

    pub fn lock_from_isr(&self, _context: &mut crate::freertos::isr::InterruptContext) -> Result<ExclusiveDataGuardIsr<T>, FreeRtosError> {
...
impl<'mutex, T: ?Sized> DerefMut for ExclusiveDataGuard<'mutex, T> {
    fn deref_mut<'a>(&'a mut self) -> &'a mut T { unsafe { &mut *self.__data.get() } }
}
