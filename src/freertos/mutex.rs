use crate::freertos::{base::*, prelude::v1::*, shim::*, units::*};
...
impl<T> MutexImpl<T, MutexRecursive> {
    ...
    pub fn new(t: T) -> Result<Self, FreeRtosError> {
        Ok(MutexImpl { mutex: MutexRecursive::create()?,
                       data:  UnsafeCell::new(t), })
    }
}
...
impl<T, M> MutexImpl<T, M> where M: MutexInnerImpl
{
    ...
    pub fn lock<D: DurationTicks>(&self, max_wait: D) -> Result<MutexGuard<T, M>, FreeRtosError> {
        ...
        Ok(MutexGuard { __mutex: &self.mutex,
                        __data:  &self.data, })
        // }
    }
...
{
    fn drop(&mut self) { self.__mutex.give(); }
}
...
impl Drop for MutexNormal {
    fn drop(&mut self) { unsafe { freertos_rs_delete_semaphore(self.0) } }
}
