use crate::freertos::{base::*, prelude::v1::*};
// use crate::freertos::task::*;
use crate::freertos::shim::*;
// use crate::freertos::mutex::*;
// use crate::freertos::queue::*;
...
impl Timer {
    ...
    pub fn new<D: DurationTicks>(period: D) -> TimerBuilder<D> {
        TimerBuilder { name: "timer".into(),
                       period,
                       auto_reload: true }
    }

    unsafe fn spawn_inner<'a>(name: &str, period_ticks: FreeRtosTickType, auto_reload: bool, callback: Box<dyn Fn(Timer) + Send + 'a>)
        ...
        extern "C" fn timer_callback(handle: FreeRtosTimerHandle) -> () {
            unsafe {
                {
                    let timer = Timer { handle, detached: true };
        ...
        Ok(Timer { handle:   timer_handle as *const _,
                   detached: false, })
    }

    fn spawn<F>(name: &str, period_tick: FreeRtosTickType, auto_reload: bool, callback: F) -> Result<Timer, FreeRtosError>
        where F: Fn(Timer) -> (),
              F: Send + 'static
    {
    ...
    pub unsafe fn detach(mut self) { self.detached = true; }

    fn get_id(&self) -> Result<FreeRtosVoidPtr, FreeRtosError> { unsafe { Ok(freertos_rs_timer_get_id(self.handle)) } }
}
    ...
    fn drop(&mut self) {
        ...
        unsafe {
            if let Ok(callback_ptr) = self.get_id() {
                // free the memory
                let _x = Box::from_raw(callback_ptr as *mut Box<dyn Fn(Timer)>);
            }
