use crate::freertos::{base::*, isr::*, prelude::v1::*, shim::*, units::*, utils::*};
...
impl TaskPriority {
    fn to_freertos(&self) -> FreeRtosUBaseType { self.0 as FreeRtosUBaseType }
}
...
impl TaskBuilder {
    ...
    pub fn start<F>(&self, func: F) -> Result<Task, FreeRtosError>
        where F: FnOnce() -> (),
              F: Send + 'static
    {
        Task::spawn(&self.task_name, self.task_stack_size, self.task_priority, func)
    }
}
...
impl Task {
    ...
    pub fn new() -> TaskBuilder {
        TaskBuilder { task_name:       "rust_task".into(),
                      task_stack_size: 1024,
                      task_priority:   TaskPriority(1), }
    }

    unsafe fn spawn_inner<'a>(f: Box<dyn FnOnce()>, name: &str, stack_size: u16, priority: TaskPriority) -> Result<Task, FreeRtosError> {
        let f = Box::new(f);
    ...
    fn spawn<F>(name: &str, stack_size: u16, priority: TaskPriority, f: F) -> Result<Task, FreeRtosError>
        where F: FnOnce() -> (),
              F: Send + 'static
    {
        unsafe {
    ...
    pub fn set_notification_value(&self, val: u32) { self.notify(TaskNotification::OverwriteValue(val)) }
    ...
    pub fn notify_from_isr(&self, context: &InterruptContext, notification: TaskNotification) -> Result<(), FreeRtosError> {
        unsafe {
            let n = notification.to_freertos();
            let t = freertos_rs_task_notify_isr(self.task_handle, n.0, n.1, context.get_task_field_mut());
            if t != 0 {
    ...
    pub fn wait_for_notification<D: DurationTicks>(&self, clear_bits_enter: u32, clear_bits_exit: u32, wait_for: D)
                                                   -> Result<u32, FreeRtosError> {
        let mut val = 0;
        let r = unsafe { freertos_rs_task_notify_wait(clear_bits_enter, clear_bits_exit, &mut val as *mut _, wait_for.to_ticks()) };
    ...
    pub fn get_stack_high_water_mark(&self) -> u32 { unsafe { freertos_rs_get_stack_high_water_mark(self.task_handle) as u32 } }
}
...
impl CurrentTask {
    ...
    pub fn get_stack_high_water_mark() -> u32 { unsafe { freertos_rs_get_stack_high_water_mark(0 as FreeRtosTaskHandle) as u32 } }
}
...
impl FreeRtosUtils {
    pub fn get_tick_count() -> FreeRtosTickType { unsafe { freertos_rs_xTaskGetTickCount() } }

    pub fn get_tick_count_duration() -> Duration { Duration::ticks(Self::get_tick_count()) }

    pub fn get_number_of_tasks() -> usize { unsafe { freertos_rs_get_number_of_tasks() as usize } }
    ...
    pub fn get_all_tasks(tasks_len: Option<usize>) -> FreeRtosSchedulerState {
        ...
        let tasks = tasks.into_iter()
                         .map(|t| FreeRtosTaskStatus { task:                  Task { task_handle: t.handle },
                                                       name:
                                                           unsafe { str_from_c_string(t.task_name) }.unwrap_or_else(|_| String::from("?")),
        ...
        FreeRtosSchedulerState { tasks, total_run_time }
    }
}
