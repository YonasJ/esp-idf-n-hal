use crate::freertos::{base::FreeRtosTickType, prelude::v1::*, shim::*};
...
impl FreeRtosTimeUnits for FreeRtosTimeUnitsShimmed {
    #[inline]
    fn get_tick_period_ms() -> u32 { unsafe { freertos_rs_get_portTICK_PERIOD_MS() } }
    #[inline]
    fn get_max_wait() -> u32 { unsafe { freertos_rs_max_wait() } }
}
...
impl<T> DurationImpl<T> where T: FreeRtosTimeUnits + Copy
{
    ...
    pub fn ms(milliseconds: u32) -> Self { Self::ticks(milliseconds / T::get_tick_period_ms()) }
    ...
    pub fn infinite() -> Self { Self::ticks(T::get_max_wait()) }

    /// A duration of zero, for non-blocking calls
    pub fn zero() -> Self { Self::ticks(0) }

    /// Smallest unit of measurement, one tick
    pub fn eps() -> Self { Self::ticks(1) }

    pub fn to_ms(&self) -> u32 { self.ticks * T::get_tick_period_ms() }
}

impl<T> DurationTicks for DurationImpl<T> where T: FreeRtosTimeUnits + Copy
{
    fn to_ticks(&self) -> FreeRtosTickType { self.ticks }
}
