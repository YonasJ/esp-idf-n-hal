use crate::freertos::{base::*, shim::*, task::*, units::*};
...
impl TaskDelay {
    ...
    /// next measurement.
    pub fn new() -> TaskDelay { TaskDelay { last_wake_time: FreeRtosUtils::get_tick_count(), } }
    ...
    pub fn delay_until<D: DurationTicks>(&mut self, delay: D) {
        unsafe {
            freertos_rs_vTaskDelayUntil(&mut self.last_wake_time as *mut FreeRtosTickType, delay.to_ticks());
        }
    }
}
...
impl TaskDelayPeriodic {
    ...
    pub fn new<D: DurationTicks>(period: D) -> TaskDelayPeriodic {
        ...
        TaskDelayPeriodic { last_wake_time: l,
    ...
    pub fn set_period<D: DurationTicks>(&mut self, period: D) { self.period_ticks = period.to_ticks(); }

    /// Reset the internal timer to zero.
    pub fn reset(&mut self) { self.last_wake_time = FreeRtosUtils::get_tick_count(); }
}
