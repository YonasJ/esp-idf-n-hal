// use crate::freertos::prelude::v1::*;
use crate::freertos::{base::*, mutex::*, queue::*, task::*, units::*};
...
impl ComputeTaskBuilder for TaskBuilder {
              ...
              R: Sync + Send + 'static
    {
        ...
        Ok(ComputeTask { task:     Task::new().start(|| {}).unwrap(),
...
impl<R: Debug> ComputeTask<R> {
    ...
    pub fn into_result<D: DurationTicks>(mut self, max_wait: D) -> Result<R, FreeRtosError> {
        ...
        let m = Arc::try_unwrap(self.result).expect("ComputeTask: Arc should have only one reference left!");
