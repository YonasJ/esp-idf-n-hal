...
impl<T: Sized + Copy> QueuePublisher<T> {
    ...
    pub fn new() -> Result<QueuePublisher<T>, FreeRtosError> {
        let inner = PublisherInner { subscribers:   Vec::new(),
                                     queue_next_id: 1, };
    ...
    pub fn subscribe<D: DurationTicks>(&self, max_size: usize, create_max_wait: D) -> Result<QueueSubscriber<T>, FreeRtosError> {
        ...
        let subscriber = SubscriberInner { id,
                                           queue,
...
impl<T: Sized + Copy> Clone for QueuePublisher<T> {
    fn clone(&self) -> Self { QueuePublisher { inner: self.inner.clone() } }
}
...
impl<T: Sized + Copy> QueueSubscriber<T> {
    ...
    pub fn receive<D: DurationTicks>(&self, max_wait: D) -> Result<T, FreeRtosError> { self.inner.queue.receive(max_wait) }
}
...
impl<T: Sized + Copy> PublisherInner<T> {
    fn unsubscribe(&mut self, subscriber: &SubscriberInner<T>) { self.subscribers.retain(|ref x| x.id != subscriber.id); }
}
