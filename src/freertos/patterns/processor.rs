use crate::freertos::{base::*, mutex::*, prelude::v1::*, queue::*, units::*};
...
impl<I> InputMessage<I> where I: Copy
{
    ...
    pub fn get_val(&self) -> I { self.val }
}

impl<I> ReplyableMessage for InputMessage<I> where I: Copy
{
    fn reply_to_client_id(&self) -> Option<usize> { self.reply_to_client_id }
}

pub struct Processor<I, O>
    where I: ReplyableMessage + Copy,
          O: Copy
{
    queue: Arc<Queue<I>>,
          ...
          O: Copy
{
    pub fn new(queue_size: usize) -> Result<Self, FreeRtosError> {
        let p = ProcessorInner { clients:        Vec::new(),
                                 next_client_id: 1, };
        let p = Arc::new(Mutex::new(p)?);
        let p = Processor { queue: Arc::new(Queue::new(queue_size)?),
                            inner: p, };
        Ok(p)
    }

    pub fn new_client(&self) -> Result<Client<I>, FreeRtosError> {
        let c = ProcessorClient { processor_queue: Arc::downgrade(&self.queue),
                                  client_reply:    (), };

        Ok(c)
    }

    pub fn new_client_with_reply<D: DurationTicks>(&self, client_receive_queue_size: usize, max_wait: D)
        ...
        let client_reply = {
            ...
            let c = ClientWithReplyQueue { id:              processor.next_client_id,
    ...
    pub fn get_receive_queue(&self) -> &Queue<I> { &*self.queue }
    ...
    pub fn reply<D: DurationTicks>(&self, received_message: I, reply: O, max_wait: D) -> Result<bool, FreeRtosError> {
        if let Some(client_id) = received_message.reply_to_client_id() {
            ...
            if let Some(client) = inner.clients
                                       .iter()
...
{
    fn remove_client_reply(&mut self, client: &ClientWithReplyQueue<O>) { self.clients.retain(|ref x| x.0 != client.id) }
}
...
impl<I, O> ProcessorClient<I, O> where I: ReplyableMessage + Copy
{
    ...
    pub fn send_from_isr(&self, context: &mut crate::freertos::isr::InterruptContext, message: I) -> Result<(), FreeRtosError> {
        let processor_queue = self.processor_queue.upgrade().ok_or(FreeRtosError::ProcessorHasShutDown)?;
...
impl<I> ProcessorClient<InputMessage<I>, ()> where I: Copy
{
    ...
    pub fn send_val_from_isr(&self, context: &mut crate::freertos::isr::InterruptContext, val: I) -> Result<(), FreeRtosError> {
          ...
          O: Copy
{
    ...
    pub fn get_receive_queue(&self) -> &Queue<O> { &self.client_reply.receive_queue }
}
...
{
    fn clone(&self) -> Self {
        ProcessorClient { processor_queue: self.processor_queue.clone(),
